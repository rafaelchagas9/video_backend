...
- Typeset the title with automatic size fitting: max 3 lines, break on word
  boundaries, tighten tracking as size grows.
...
The resolved title pairing is Fraunces (`wght=380`, display optical size) with
Archivo (`wght=400`) as the fallback. Both files must be available locally to
the backend so rendering never depends on a network font request.
