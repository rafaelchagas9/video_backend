...
export async function storyboardsRoutes(
  ...
  fastify.get(
    "/videos/:id/thumbnails.vtt",
    {
      schema: {
        ...
        description: "Returns the WebVTT file with storyboard sprite coordinates for Vidstack slider preview.",
      },
    },
    async (request, reply) => {
  ...
  fastify.get(
    "/videos/:id/storyboard.jpg",
    {
      schema: {
        ...
        description: "Returns the JPEG sprite sheet image for slider preview thumbnails.",
      },
    },
    async (request, reply) => {
      const { id } = request.params as { id: string };
      const spriteBuffer = await storyboardsService.getSpriteBuffer(Number(id));

      reply.header("Content-Type", "image/jpeg");
      reply.header("Cache-Control", "public, max-age=86400");
      return reply.send(spriteBuffer);
    },
  );
  ...
  app.delete(
    "/videos/:id/storyboard",
    {
      schema: {
        ...
        description: "Deletes the storyboard files and database record for a video.",
        params: idParamSchema,
        response: {
  ...
  app.get(
    ...
    async (request, reply) => {
      const storyboard = await storyboardsService.findByVideoId(request.params.id);
