...
describe("demo artwork fixtures", () => {
  ...
  test("reuses safe raster fixtures without assigning stale titles to expanded videos", () => {
    const source = demoMockService.getVideoById(1);
    const expanded = demoMockService.getVideoById(45);
  ...
  test("returns summaries and resolves image ids only to demo_mode files", () => {
    const videoIds = demoMockService
      .getVideos({ limit: 200 })
    ...
    expect(summaries.get(1)).toMatchObject({
      urls: {
        card: expect.stringMatching(/^\/api\/artwork\/11\/image\?h=[a-f0-9]{16}$/),
        title: expect.stringMatching(/^\/api\/artwork\/15\/image\?h=[a-f0-9]{16}$/),
      },
